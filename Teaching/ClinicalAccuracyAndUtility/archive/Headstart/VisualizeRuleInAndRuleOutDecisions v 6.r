...
DxStats <- function(n, prevalence, sensitivity, specificity) {
  ...
  PreTestOddsN <- (1 - prevalence)/prevalence
...
ui<-fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput(inputId = "DxCondition", label = "Condition", value = "COPD exacerbation"),
      textInput(inputId = "DxTestName", label = "Name of test", value = "Headstart"),
      ...
      textInput(inputId = "DxRuleInDecision", label = "Rule-in decision", value = "follow PMP"),
      numericInput("RuleInDecisionThreshold", "Rule-in PPV threshold", min=0, max=1, value= 0.5, step = 0.01),
      textInput(inputId = "IndeterminateDecision", label = "retest within 12 to 24 hours", value = "repeat Headstart in 12-24 hrs"),
      textInput(inputId = "DxRuleOutDecision", label = "Rule-out decision", value = "continue usual testing"),
...
#==========================================================
    ...
    graphPre2PostProb <- eventReactive(input$GoButton, {
      x <- seq(from = 0, to = 1, by = 0.05) ### preTest probability along the x-axis
