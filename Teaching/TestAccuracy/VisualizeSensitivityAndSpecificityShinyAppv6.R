...
server<-function(input, output) {
    ...
    contingencyM<- reactive({return({as.data.frame(cmX = cmX(), cmY = cmY(), row.names = rnames())})})
  ...
  output$testedPlots<-renderPlot( {
    ...
    if (input$sorted) {
        ...
        geom_segment(aes(x = hx3, y = hy3, xend = hx4, yend = hy4, colour = NULL, shape = NULL), 
                       data = hdf2()) 
       p2 <- p2 + geom_text(data = contingencyM(), mapping = aes(x=cmX, y=cmY, label = rownames(contingencyM())))
    }
