...
#      p1 <- p1 + geom_line(color = "red", data = vLine(), aes(x = x, y = y), size = 0.8) #### ??? why does this cause an error? 
  ...
  output$testedPlots<-renderPlot( {
    ...
    if (input$sorted) {
      p2 <- ggplot(populationdf(), aes(x=xsorted, y=ysorted, color=testResult, shape = testResult)) + geom_point(size = 4) + geom_line(data = hLine(), aes(x = x, y = y), size = 0.8, color = "red") + geom_line(data = vLine(), aes(x = x, y = y), size = 0.8, color = "red") 
      # p2 <- p2 + geom_line(data = vLine(), aes(x = x, y = y), size = 0.8, color = "red") #### ??? why does this cause an error? 
      # p2 <- p2 + geom_line(data = hLine(), aes(x = x, y = y), size = 0.8, color = "red") #### ??? why does this cause an error? 
      
    } else {
      p2 <- ggplot(populationdf(), aes(x=xsorted, y=yunsorted, color=condition, shape = testResult)) + geom_point(size = 4)
    }
