...
#########################################################
  ...
  urlTab1 <- urlTab1 <- "https://onedrive.live.com/download?cid=B2035DBFA124EFE7&resid=B2035DBFA124EFE7%213030&authkey=AH5-Q7iD0Rd4_aw"
  ...
  urlTab4 <- urlTab4 <- ""
  tab4Html <- content(GET(urlTab1), "text", encoding = "ISO-8859-1")  
  
  # tab1Html <- read_file("https://onedrive.live.com/embed?cid=B2035DBFA124EFE7&resid=B2035DBFA124EFE7%213030&authkey=AH5-Q7iD0Rd4_aw")
  # tab2Html <- read_file("tab2.html")
  # tab4Html <- read_file("tab4.html")

#################### ui ###############################

ui<-fluidPage(

  # NICE DAP manual section 
  # 14. Modelling clinical outcomes and the cost effectiveness
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("RuleInDecisionThreshold", "Rule-in post +ve test threshold", min=0, max=1, value= 0.5, step = 0.01),
      ...
      numericInput("RuleOutDecisionThreshold", "Rule-out post -ve test threshold", min=0, max=1, value= 0.1, step = 0.01)
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("About", HTML(tab1Html)),
        tabPanel("About graph 1",HTML(tab2Html)),
      tabPanel("Graph 1: Post-test probabilities",
      plotOutput("PrePostProb")
      ),
      tabPanel("About graph 2",HTML(tab4Html)),
      tabPanel("Graph 2: Decision thresholds",
...
server<-function(input, output) {
    ...
    PostTestProbP <- eventReactive(input$GoButton, {PostTestOddsP()/(PostTestOddsP() + 1)}) # = PPV
    PostTestProbN <- eventReactive(input$GoButton, {PostTestOddsN()/(PostTestOddsN() + 1)}) # = (1 - NPV)
    ...
    linesDf <- eventReactive(input$GoButton, {
      ...
      data.frame(
        ...
        TestPosX <- c(0, 1),
        TestPosY <- c(input$prevalence, Dx$PPV),
...
### coordinates and labels for plot 1
    ...
    postTestLabels <- eventReactive(input$GoButton, {
      ...
      data.frame(
        ...
        y = c(
          PPV() + 0.025,           # = post +ve test probability
        ...
        labels = c(
          strwrap(paste0("Prob post +ve test = ", round(PPV()*100), "%"), 40),
...
#==========================================================
  ...
  output$PrePostProb<-renderPlot({
    ggplot(graphPre2PostProb()) +
      geom_line(aes(
        x = graphPre2PostProb()$x, 
        y = graphPre2PostProb()$yP), 
      ...
      geom_line(aes(
        x = graphPre2PostProb()$x,
        y = graphPre2PostProb()$yN),
        data = graphPre2PostProb(), stat = "identity", position = "identity") +
      geom_ribbon(data = graphPre2PostProb(),
                  aes(x = graphPre2PostProb()$x,
      ...
      labs(
        x = "Pre-test probability (prevalence)",
        y = paste0("Post test probability after ", DxTestName())) +
      ...
      geom_line(data = linesPre2PostProb(), aes(
        ...
        y = linesPre2PostProb()$PostProbPosY),
        stat = "identity", position = "identity") +
      ...
      geom_line(data = linesPre2PostProb(), aes(
        ...
        y = linesPre2PostProb()$PostProbNegY),
        stat = "identity", position = "identity") +
      geom_ribbon(data = linesPre2PostProb(), aes(
        x = linesPre2PostProb()$PostProbNegX,
