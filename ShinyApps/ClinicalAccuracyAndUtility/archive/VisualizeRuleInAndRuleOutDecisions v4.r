...
ciproportion <- function(y,n) {
  ...
  h <- 2 # continuity correction
# use ciALTx: Adjusted Logit-Wald method of CI estimation
  return(data.frame(
    # cill <- ciALTx(y, n, alpha, h)$LALTx,  # logit lower 95% confidence limit
    # ciul <- ciALTx(y, n, alpha, h)$UALTx  # logit upper 95% confidence limit
    ciul <- 0.1528,
    cill <- 0.0358
  ))
}
  ...
  #  )
    ...
    linesDf <- eventReactive(input$GoButton, {
        ...
        return({data.frame(
          ...
          TestPosX <- c(0, 1),
          TestPosY <- c(prevalence(), Tp()/(Tp() + Fp())),
          
          TestNegX <- c(0, 1),
          TestNegY <- c(prevalence(), 1 - Tn()/(Tn() + Fn())),

          TPY_cil <- c(prevalence(), ciproportion(TestPosY(), Dpos())$cill),
          TPY_ciu <- c(prevalence(), ciproportion(TestPosY(), Dpos())$ciul),
